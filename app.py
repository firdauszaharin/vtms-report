...
def sharpen_image(img, level="high"):
    """
    Sharpen image using UnsharpMask + ImageEnhance combination
    ...
    try:
        ...
        # Step 1: Apply UnsharpMask filter
        img = img.filter(ImageFilter.UnsharpMask(
            radius=s["unsharp"][0], 
            percent=s["unsharp"][1], 
            threshold=s["unsharp"][2]
        ))
        ...
        # Step 3: Slightly boost contrast for clarity
    ...
    if not p_img or not v_img: st.error("Please provide both signatures!")
        ...
        if evidence_data:
            ...
            for i, ev in enumerate(evidence_data):
                ...
                # Use ImageOps.contain to fit entire image without cropping
                img = ImageOps.contain(img, (800, 550), method=Image.Resampling.LANCZOS)
                
                # Create white background canvas and paste image centered
                ...
                # Apply sharpening
                img = sharpen_image(img, level="high")
                
                temp_filename = f"tmp_ev_{i}.jpg"
                img.save(temp_filename, quality=95)
                
                # Place image inside box with padding
        ...
        # Convert to base64 and open in new tab
        ...
        st.success("✅ Report generated! PDF dibuka dalam tab baru.")
